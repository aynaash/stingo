...
describe('builder', () => {
  test('camera() makes a camera scene', async () => {
    const { film, camera } = await import('@hersidev/stingo');
  ...
  test('.camera() composites a take onto any other block', async () => {
    const { film, code } = await import('@hersidev/stingo');
...
describe('a take shorter than its scene', () => {
  // docs/content/camera.md promises "A take that runs out before its scene does
  // holds on its last frame rather than failing the render". It used to hold for
  // a small overshoot and go black for a large one, because a seek past the clip
  // decodes nothing and leaves no frame to hold.
  const TAKE = '.stingo/gallery/standin.mp4';
  const box = { w: 240, h: 426 };
  const luma = (b: Buffer) => {
    let t = 0;
    for (let i = 0; i < b.length; i += 4) t += b[i]! + b[i + 1]! + b[i + 2]!;
    return t / ((b.length / 4) * 3);
  };

  test('holds its tail however far past the end it is asked', async () => {
    if (!(await Bun.file(TAKE).exists())) return;   // generated by tools/gallery.ts
    const { VideoSource } = await import('@stingo/media');
    const s = await VideoSource.open(TAKE, 30);
    const inside = luma(await s.at(5, box));
    expect(inside).toBeGreaterThan(5);
    for (const t of [12.5, 20, 120]) {
      expect(luma(await s.at(t, box)), `${t}s past a 12s take`).toBeGreaterThan(5);
    }
    await s.close?.();
  }, 30000);

  test('holds even when the very first request is past the end', async () => {
    if (!(await Bun.file(TAKE).exists())) return;
    const { VideoSource } = await import('@stingo/media');
    const s = await VideoSource.open(TAKE, 30);
    expect(luma(await s.at(99, box)), 'cold source seeked past the end').toBeGreaterThan(5);
    await s.close?.();
  }, 30000);
});
